...
{	
	//ESP_LOGI(TAG,"g_fft_index %d",g_fft_index);
	//缓冲区写入结束
	if(g_fft_index>=FFT_N)
	{
		//开始变换显示
		...
		//检查HbO2和Hb的变化频率是否一致
		if(1)//(s1_max_index == s2_max_index)
		{
			//心率计算
		...
		ESP_LOGI(TAG,"heart:%d SpO2:%f",g_blooddata.heart,g_blooddata.SpO2);
		//结束变换显示
